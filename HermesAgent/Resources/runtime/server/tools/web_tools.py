...
def _ensure_web_provider_plugins_discovered() -> None:
    """Ensure bundled/user web provider plugins have registered providers.

    ``model_tools`` normally discovers plugins at startup, but direct tool
    execution paths (gateway/tool smoke tests and some embedded runtimes) can
    reach ``tools.web_tools`` before that import side effect happens. The web
    provider registry is process-local, so make discovery idempotent at the
    point of use before looking up Tavily/Firecrawl/etc.
    """
    try:
        from hermes_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as e:
        logger.debug("Web provider plugin discovery skipped: %s", e)
    ...
    try:
        ...
        for module_name in (
            ...
            try:
                ...
                logger.debug("Bundled web provider registration skipped for %s: %s", module_name, exc)
    except Exception as e:
        logger.debug("Bundled web provider fallback discovery skipped: %s", e)
